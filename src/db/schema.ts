...
/**
 * 食事の記録。**写真とメモを同じものとして持つ。**
 *
 * **飾るためではなく、見返すため。**一度やめた機能だが（作り置きの皿は映えない）、
 * 目的が「あとで自分が見返す」に変わったので、別のものとして入れ直した。
 *
 * 写真だけの回もあれば、**文だけの回もある**（撮り忘れた、撮る場面ではなかった、
 * 味の覚書だけ残したい）。別々のテーブルにすると、見返すときに2つ並べて
 * 時系列を頭で合わせることになるので、**1つにまとめて時刻で並べる**。
 *
 * レシピではなく、**食べた1回**に紐づける。同じ料理でも、作った日ごとに記録は別物。
 * 献立に無いもの（間食）も置けるように、`plannedMealId` は任意にしてある。
 *
 * 画像は Blob。**書き出し（バックアップ）には含めない**——JSON に入らないうえ、
 * 数十枚で数MBになり、書き出したファイルが持ち運べなくなる。
 *
 * ※ 表の名前は `mealPhotos` のまま。**中身の意味が広がっても、
 *   機械が読む名前は変えない**——変えると保存済みの記録が消える。
 */
export interface MealLog extends Entity {
  date: ISODate;
  slot: MealSlot;
  /** 献立の食事に紐づくなら。間食など、献立に無いものは持たない */
  plannedMealId?: UUID;
  profileId?: UUID;
  /** 長辺1280pxまで縮めた JPEG。文だけの記録では持たない */
  image?: Blob;
  width?: number;
  height?: number;
  /** ひとこと。写真に添えることも、これだけを残すこともできる */
  note?: string;
  takenAt: ISODateTime;
}
...
export interface CalendarSettings {
   ...
   * 書く先は「プレノラ」という専用カレンダーだけ。他のカレンダーには触れない。
